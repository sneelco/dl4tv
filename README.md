...
2. In the dl4tv UI you map each playlist to a download folder.
...
services:
  dl4tv:
    ...
    ports:
      - "8484:8484"
    environment:
      DL4TV_PUBLIC_URL: "http://localhost:8484"   # must match your OAuth redirect URI
...
dl4tv can talk to YouTube two ways. **OAuth** is the one you want if you plan to
use private playlists or want dl4tv to list your playlists for you.
...
An API key is enough for **public** playlists that you add by URL. Create one
under **Credentials → Create credentials → API key**, paste it into Settings,
and use the *Add a playlist by link* box. dl4tv cannot list your account's
playlists in this mode.
...
**Playlists tab → Load from YouTube** lists everything on the connected
account, including your uploads and liked videos. Type a folder next to a
playlist and click **Map**.

Or paste any of these into *Add a playlist by link*:
...
| `network` | Timeouts, DNS, 5xx | yes, up to *Attempts before giving up* |
